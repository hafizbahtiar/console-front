...
import { ApiClientError, getAccessToken } from "@/lib/api-client"
...
export default function QueueMonitoringPage() {
    ...
    // Get API URL - handle both with and without /api/v1 prefix
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'
    // Remove trailing /api/v1 if present to avoid double prefix
    const baseUrl = apiUrl.replace(/\/api\/v1$/, '')

    // Get access token for Bull Board iframe authentication (reactive to token changes)
    const bullBoardUrl = useMemo(() => {
        const accessToken = getAccessToken()
        return accessToken
            ? `${baseUrl}/api/v1/admin/queues?token=${encodeURIComponent(accessToken)}`
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                setIsLoading(true)
                const [statsResult, failedResult, completedResult] = await Promise.allSettled([
                    getQueueStats(),
                    selectedQueue ? getFailedJobs(selectedQueue, 0, 10) : Promise.resolve({ jobs: [], total: 0 }),
                    selectedQueue ? getJobHistory(selectedQueue, 'completed', 0, 10) : Promise.resolve({ jobs: [], total: 0 }),
                ])

                // Handle queue stats result
                if (statsResult.status === 'fulfilled') {
                    const stats = statsResult.value
                    setQueueStats(stats)

                    // Set first queue if not already selected
                    if (stats && Object.keys(stats).length > 0 && !selectedQueue) {
                        const firstQueue = Object.keys(stats)[0]
                        setSelectedQueue(firstQueue)
                    }
                } else {
                    console.error('Failed to fetch queue stats:', statsResult.reason)
                    const errorMessage =
                        statsResult.reason instanceof ApiClientError
                            ? statsResult.reason.message
                            : 'Failed to load queue statistics'
                    toast.error(errorMessage)
                }

                // Handle failed jobs result
                if (failedResult.status === 'fulfilled') {
                    setFailedJobs(failedResult.value.jobs || [])
                } else {
                    console.error('Failed to fetch failed jobs:', failedResult.reason)
                    // Don't show error toast for failed jobs as it's optional
                }

                // Handle completed jobs result
                if (completedResult.status === 'fulfilled') {
                    setCompletedJobs(completedResult.value.jobs || [])
                } else {
                    console.error('Failed to fetch completed jobs:', completedResult.reason)
                    // Don't show error toast for completed jobs as it's optional
                }
            } catch (error: any) {
    ...
    const handleRetryJob = async (queueName: string, jobId: string) => {
        try {
            ...
            const statsResult = await Promise.allSettled([
                getQueueStats(),
                getFailedJobs(queueName, 0, 10),
            ])

            if (statsResult[0].status === 'fulfilled') {
                setQueueStats(statsResult[0].value)
            }

            if (statsResult[1].status === 'fulfilled') {
                setFailedJobs(statsResult[1].value.jobs || [])
            }
        } catch (error: any) {
    ...
    const handleCleanJobs = async (queueName: string, status: 'completed' | 'failed' | 'all') => {
        try {
            ...
            toast.success(`Cleaned ${result.cleaned} ${status} job(s) from ${queueName} queue`)
            // Refresh data
            const refreshResults = await Promise.allSettled([
                getQueueStats(),
                getFailedJobs(queueName, 0, 10),
                getJobHistory(queueName, 'completed', 0, 10),
            ])

            if (refreshResults[0].status === 'fulfilled') {
                setQueueStats(refreshResults[0].value)
            }

            if (refreshResults[1].status === 'fulfilled') {
                setFailedJobs(refreshResults[1].value.jobs || [])
            }

            if (refreshResults[2].status === 'fulfilled') {
                setCompletedJobs(refreshResults[2].value.jobs || [])
            }
        } catch (error: any) {
            ...
            const message =
                error instanceof ApiClientError
                    ? error.message
                    : 'Failed to clean jobs. Please try again.'
            toast.error(message)
        }
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Queue Monitoring</h1>
