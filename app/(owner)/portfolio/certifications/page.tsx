...
export default function PortfolioCertificationsPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setCertifications((prev) => prev.filter((c) => c.id !== certificationToDelete.id))
  ...
  const handleBulkDeleteClick = (certifications: Certification[]) => {
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      const ids = selectedCertifications.map((c) => c.id)
      // Try bulk delete first, fallback to individual deletes if endpoint doesn't exist
      try {
        const result = await bulkDeleteCertifications(ids)
        if (result.failedIds.length > 0) {
          toast.warning(
            `Deleted ${result.deletedCount} certification(s). ${result.failedIds.length} failed.`
          )
        } else {
          toast.success(`Successfully deleted ${result.deletedCount} certification(s)`)
        }
        setCertifications((prev) => prev.filter((c) => !ids.includes(c.id)))
      } catch (bulkError) {
        // Fallback to individual deletes if bulk endpoint doesn't exist
        const deletePromises = ids.map((id) => deleteCertification(id))
        await Promise.all(deletePromises)
        setCertifications((prev) => prev.filter((c) => !ids.includes(c.id)))
        toast.success(`Successfully deleted ${ids.length} certification(s)`)
      }
      ...
      const message =
        error instanceof ApiClientError
          ? error.message
          : "Failed to delete certifications. Please try again."
      toast.error(message)
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete "${certificationToDelete?.name}" from ${certificationToDelete?.issuer}? This action cannot be undone.`}
        confirmText="Delete"
      ...
      <ConfirmDialog
        ...
        title="Delete Multiple Certifications"
        description={`Are you sure you want to delete ${selectedCertifications.length} selected certification(s)? This action cannot be undone.`}
        confirmText={`Delete ${selectedCertifications.length} Certification(s)`}
        confirmVariant="destructive"
